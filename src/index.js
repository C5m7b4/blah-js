...
const displayCheapestItem = () => {
  ...
  div.id = divName;
  div.innerHTML = `The cheapest item is ${cheapest.name} and it it ${cheapest.price}`;
...
const displayMostExpensive = () => {
  ...
  const div = document.createElement('div');
  div.id = divName;
  div.innerHTML = `The most expensive item is ${highest.name} and it it ${highest.price}`;
  parent.appendChild(div);
};
